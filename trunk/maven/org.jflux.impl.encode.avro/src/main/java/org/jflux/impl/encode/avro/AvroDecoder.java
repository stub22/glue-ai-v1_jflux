...
public class AvroDecoder<S extends InputStream, T extends IndexedRecord>
	...
	private static final Logger theLogger = LoggerFactory.getLogger(AvroDecoder.class);
	...
	AvroDecoder<S, R> buildBinaryDecoder(
		...
		return new AvroDecoder<>(outputClass, schema, false);
	}
	...
	AvroDecoder<S, R> buildJsonDecoder(
		...
		return new AvroDecoder<>(outputClass, schema, true);
	}
	...
	AvroDecoder(Class<T> clazz, Schema schema, boolean json, ClassLoader classLoader) {
		...
		if (myReader == null && schema != null) {
			myReader = new GenericDatumReader<>(schema);
		}
		if (myReader == null) {
			throw new NullPointerException();
		}
	...
	public T adapt(S a) {
		...
		try {
			...
			theLogger.warn("There was an error decoding the stream.", ex);
			return null;
		}
	}
}
