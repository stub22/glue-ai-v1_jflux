...
public class AddOnUtils {
	private static final Logger theLogger = LoggerFactory.getLogger(AddOnUtils.class);
	...
	public static ServiceRegistration registerAddOnDriver(
		...
		if (reg == null) {
			theLogger.warn("Unknown error occured while registering "
					+ "ServiceConfigurationWriter.");
		} else {
			theLogger.info("ServiceAddOnDriver registered successfully.");
		}
		return reg;
	}
	...
	public static ServiceReference[] getAddOnDriverReferences(
		...
		if (refs == null || refs.length == 0) {
			theLogger.warn("Unable to find AddOnDriver with "
							+ "serviceVersion={} and configFormat={}",
	...
	public static ServiceReference[] getAddOnDriverReferences(
		...
		if (refs == null || refs.length == 0) {
			theLogger.warn("Unable to find AddOnDrivers");
			return null;
		}
		return refs;
	}
	...
	public static List<ServiceAddOnDriver> getAddOnDriverList(
		...
		List<ServiceAddOnDriver> drivers = new ArrayList<>();
