...
public class OSGiUtils {
	private static final Logger theLogger = LoggerFactory.getLogger(OSGiUtils.class);

	/**
	 * Returns the BundleContext associated with the given Class.
	 *
	 * @param clazz Class associated with a Bundle
	 * @return the BundleContext associated with the given Class, if no Bundle is found for the
	 * Class null is returned
	 */
	public static BundleContext getBundleContext(Class clazz) {
		Bundle bundle = FrameworkUtil.getBundle(clazz);
		if (bundle == null) {
			theLogger.warn("Unable to find Bundle for class {}",
					clazz.getName());
			return null;
		}
		try {
			...
			if (context == null) {
				theLogger.warn("Found Bundle for class {}, "
						+ "but no valid BundleContext.", clazz.getName());
			}
			return context;
		} catch (SecurityException ex) {
			//Not actually catching, ensuring the SecurityException is logged.
			theLogger.error("Permission Denied to BundleContext for class '{}'" +
					".  Caller lacks appropriate AdminPermissions.", clazz.getName(), ex);
			throw ex;
		}
	}
	 ...
	 * @return true if a matching ServiceReference is found, returns null if there is an
	...
	public static Boolean serviceExists(
		...
		try {
			...
			theLogger.warn("Invalid OSGi filter syntax String: {}.", filter, ex);
			return null;
		}
	}
	 ...
	 * @return true if a ServiceReference is found matching one or more of the given classes and the
	 * filter String, return null if there is an InvalidSyntaxException
	 */
	public static Boolean serviceExists(
		...
		for (String c : classes) {
			...
			if (b == null) {//InvalidSyntaxException
				...
				theLogger.info("Found existing Service: {}, matching {}.",
						c, filter);
				return true;
			}
		}
		theLogger.info("No existing Service matching {} with class {}.",
				filter, Arrays.toString(classes));
		return false;
	}
	...
	public static <T> T getService(
		...
		if (context == null || ref == null) {
			theLogger.warn("Cannot get service with null "
		...
		try {
			...
			theLogger.trace("Unable to get service.", ex);
		}
		if (obj == null) {
			theLogger.trace("Service for reference ({}) has a null Service.",
					getInformationString(ref));
			context.ungetService(ref);
			return null;
		} else if (!clazz.isAssignableFrom(obj.getClass())) {
			theLogger.warn("Service for reference ({}) is not assignable to {}.",
					getInformationString(ref), clazz.getName());
			context.ungetService(ref);
			return null;
		}
		try {
			...
			theLogger.warn("There was an error casting the Service", t);
			context.ungetService(ref);
			return null;
		}
	}
	...
	public static <T> SingleServiceListener<T> createIdServiceListener(
		...
		return new SingleServiceListener<>(clazz, context, filter);
	}
	...
	public static ServiceRegistration registerService(
		...
		if (reg == null) {
			theLogger.warn("Unknown error registering Service with Id: {}={}",
					idPropertyName, idString);
		} else {
			theLogger.info("Successfully registered Service with Id: {}={}",
					idPropertyName, idString);
		}
		return reg;
	}
	...
	public static ServiceRegistration registerUniqueService(
		...
		if (serviceExists(context, className,
			...
			theLogger.warn("Service already exists with Id: {}={}",
					idPropertyName, idString);
			return null;
		}
	...
	public static void addServiceListener(
		...
		listenerFilter = empty ? listenerFilter : "(&" + listenerFilter +
				serviceFilter + ")";
		theLogger.info("Adding ServiceListener for: {}", listenerFilter);
