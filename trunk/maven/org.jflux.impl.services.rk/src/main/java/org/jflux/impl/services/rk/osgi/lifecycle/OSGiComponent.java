...
import org.jflux.impl.services.rk.lifecycle.ServiceLifecycleProvider.Validator;
...
import static org.jflux.impl.services.rk.lifecycle.ServiceLifecycleProvider.PROP_DEPENDENCY_ADDED;
import static org.jflux.impl.services.rk.lifecycle.ServiceLifecycleProvider.PROP_DEPENDENCY_REMOVED;
import static org.jflux.impl.services.rk.osgi.lifecycle.ServiceDependenciesTracker.PROP_ALL_DEPENDENCIES_AVAILABLE;
import static org.jflux.impl.services.rk.osgi.lifecycle.ServiceDependenciesTracker.PROP_DEPENDENCY_AVAILABLE;
import static org.jflux.impl.services.rk.osgi.lifecycle.ServiceDependenciesTracker.PROP_DEPENDENCY_UNAVAILABLE;
...
public class OSGiComponent<T> extends
	...
	private static synchronized ThreadPoolExecutor getExecutor() {
		if (theExecutor == null) {
			...
			theExecutor.setKeepAliveTime(20, TimeUnit.SECONDS);
	 ...
	 * @param context           BundleContext for accessing the OSGi Service Registry
	 * @param lifecycle         lifecycle provider for the managed service
	 * @param registrationProps optional properties to be used when registering the managed service
	 *                          to the OSGi Service Registry.  These are combined with any
	 *                          properties from the ServiceLifecycleProvider
	 */
	public OSGiComponent(BundleContext context,
						 ServiceLifecycleProvider<T> lifecycle,
	 ...
	 *                          to the OSGi Service Registry.  These are combined with any
	 *                          properties from the ServiceLifecycleProvider
	 * @param selfRegister      determines if this OSGiComponent should register itself when
	 *                          started, by default this is true
	 */
	public OSGiComponent(BundleContext context,
						 ServiceLifecycleProvider<T> lifecycle,
	 ...
	 * @param context                BundleContext for accessing the OSGi Service Registry
	 * @param lifecycle              lifecycle provider for the managed service
	 * @param registrationProps      optional properties to be used when registering the managed
	 *                               service to the OSGi Service Registry.  These are combined with
	 *                               any properties from the ServiceLifecycleProvider
	 * @param registrationClassNames class names to be used when registering the managed service to
	 *                               the OSGi Service Registry.  By default, this value is taken
	 *                               from the ServiceLifecycleProvider's getServiceClass().
	 */
	public OSGiComponent(BundleContext context,
						 ServiceLifecycleProvider<T> lifecycle,
						 Properties registrationProps,
	...
	public void dispose() {
		...
		getLogger().debug("Component Detail {}", myService);
		stopLifecycle();
		clearAllListeners();
		if (myDependenciesTracker != null) {
	...
	public void __unregister() {
		if (myServiceRegistration != null) {
			try {
				...
				myServiceRegistration = null;
				if (myService != null) {
					getLogger().info("Service Successfully Unregistered, class= {}", myService.getClass());
	...
	private boolean uniquePropertiesAvailable() {
		...
		for (Entry e : myUniqueProperties.entrySet()) {
			...
			if (OSGiUtils.serviceExists(
				...
				getLogger().warn("Unique service property already in use: ({}={}).", key, val);
				return false;
			}
		}
		return true;
