...
public class ConnectionUtils {
	private static final Logger theLogger = LoggerFactory.getLogger(ConnectionUtils.class);
	/**
	 ...
	 * @return true if there is a connection with the given id, either pre-existing or the given
	 * connection, false if a connection is not available (one did not previously exist and
	 * registration failed)
	 */
	public static ServiceRegistration ensureSession(BundleContext context,
		...
		if (OSGiUtils.serviceExists(context, Session.class.getName(),
			...
			theLogger.info("Connection for Id: {} alreaday exists.  "
		...
		try {
			...
			theLogger.warn("Unable to create session.", ex);
			return null;
		}
		ServiceRegistration reg = OSGiUtils.registerUniqueService(context,
				...
				conId, session, props);
		if (reg == null) {
			theLogger.warn("Unable to register connection with Id: {}.", conId);
			return null;
		}
		//do something with reg
		theLogger.info("Connection with Id: {} registered successfully.", conId);
		return reg;
	}
	...
	public static boolean ensureConnection(BundleContext context,
		...
		if (OSGiUtils.serviceExists(context, Connection.class.getName(),
				ConnectionManager.PROP_CONNECTION_ID, conId, props)) {
			theLogger.info("Connection for Id: {} already exists.  "
					+ "No need to create new Connection.", conId);
			return true;
		}
	 ...
	 * @param props    optional OSGi registration properties
	 * @return true if there is a destination with the given id, either pre-existing or newly
	 * created, false if a destination is not available (one did not previously exist and
	 * registration failed)
	 */
	public static ServiceRegistration ensureDestination(BundleContext context,
		...
		if (OSGiUtils.serviceExists(context, Destination.class.getName(),
			...
			theLogger.info("Destination for Id: {} already exists.  "
		...
		if (dest == null) {
			theLogger.warn("Unable to create destination with Id: {}.", destId);
			return null;
		}
		ServiceRegistration reg = ConnectionManager.registerDestination(
				context, destId, dest, props);
		if (reg == null) {
			theLogger.warn("Unable to register destination with Id: {}.", destId);
			return null;
		}
		//do something with reg
		theLogger.info("Destination with Id: {} registered successfully.", destId);
		return reg;
	}
	 ...
	 *
	 * @param context    BundleContext for OSGi
	 * @param destParams parameters for calls to <code>ensureDestination</code>, must be in
	 *                   multiples of 4 with the correct types
	 */
	public static Set<ServiceRegistration> ensureDestinations(
			BundleContext context, Object... destParams) {
		Set<ServiceRegistration> regs = new HashSet<>();
