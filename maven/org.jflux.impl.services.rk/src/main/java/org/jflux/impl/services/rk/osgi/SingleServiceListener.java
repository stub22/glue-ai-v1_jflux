...
public class SingleServiceListener<T>
	...
	private static final Logger theLogger = LoggerFactory.getLogger(SingleServiceListener.class);
	 ...
	 *
	 * @return reference to the service being tracked, null if the service is not available
	 */
	...
	private boolean startListening() {
		...
		try {
			...
			theLogger.warn("Could not register ServiceListener. "
	...
	private void stopListening() {
		try {
			...
			theLogger.warn("BundleContext not valid.", ex);
		}
	}
	...
	private void untrackWithEvent(ServiceReference ref) {
		...
		try {
			...
			theLogger.debug("Cannot unget service.  Bundle already uninstalled, no need to unget.");
		} catch (Exception ex) {
			theLogger.warn("Error ungetting service", ex);
		}
		myReference = null;
